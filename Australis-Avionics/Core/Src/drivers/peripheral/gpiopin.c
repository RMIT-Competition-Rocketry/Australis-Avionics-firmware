...
GPIOpin_t GPIOpin_init(GPIO_TypeDef *port, GPIO_Pin pin, GPIO_Config *config) {
  ...
  int portIndex;
  // Get index of supplied port (i.e. GPIOA -> 0) by subtracting the base address
  // of GPIOK (first port in memory map), dividing by the number of bytes per GPIO,
  // and invert the index by subtracting it from the max port index and taking the
  // absolute.
  //
  // portIndex = | GPIO_max - ((port - GPIO_min) / size(GPIO)) |
  portIndex      = ((uint32_t)port - GPIOK_BASE) / sizeof(GPIO_TypeDef); // Get index of port in memory map
  portIndex      = GPIO_MAX_PORT - portIndex;                            // Subtract memory map index from max index
  portIndex      = abs(portIndex);                                       // take the absolute
  ...
  port->OTYPER  |= (config->mode << pin);                                // Shift in type bits from config
